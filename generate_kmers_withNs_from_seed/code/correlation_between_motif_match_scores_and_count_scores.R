...
correlation_data %>% filter((spearman_estimate-0.5)<0.001) %>% pull(ID) %>% head() #BARX1_HT-SELEX_TATTCG20NCTG_AC_NYNATTAN_1_3
...
correlation_data %>% filter(spearman_estimate==min(correlation_data$spearman_estimate)) %>% pull(ID) #PAX7_HT-SELEX_TCGTTT40NGTC_KAF_NNATTCGTCACGSTN_1_4


correlation_data %>% filter(abs(spearman_estimate+0.5)<0.1) %>% pull(ID) %>% head() #BARX1_HT-SELEX_TATTCG20NCTG_AC_NYNATTAN_1_3
