...
successfull=data.frame(ID=gsub(".tsv.gz","",dir("/scratch/project_2013895/SELEX/streamed_kmers_scaled_by_maxscore_included")
        )
  )
...
hist_path="/scratch/project_2013895/SELEX/score_histograms/"

for(i in 1:nrow(successfull)){
  #i=1
  print(i)
  info$ID[i]=successfull$ID[i]
  data=try(read_delim(paste0("/scratch/project_2013895/SELEX/streamed_kmers/", successfull$ID[i],".tsv"), delim="\t", col_names=FALSE), silent=TRUE)
  ...
  data=try(read_delim(paste0("/scratch/project_2013895/SELEX/streamed_kmers_scaled_by_maxscore/", successfull$ID[i],".tsv"),delim="\t",col_names=FALSE), silent=TRUE)
...
write.table(info, 
            "/projappl/project_2013895/SELEX/generate_kmers_from_seed/code/score_analysis.tsv",
            quote=FALSE, 
            sep="\t",
            row.names=FALSE,
            col.names=TRUE)
...
info=read_delim("/projappl/project_2013895/SELEX/generate_kmers_from_seed/code/score_analysis.tsv")
info$kmer_nro %>% is.na() %>% table()
